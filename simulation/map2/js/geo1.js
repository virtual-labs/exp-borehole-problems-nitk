	 ...
	 $('#mycanvas').mousemove(function(event){
		 ...
		 if(sp1==1)
		 {
			  ...
			  else if(xx>=644 && xx<=649 && yy>=425 && yy<=430)
			 {
			  ...
			  else if(xx>=385 && xx<=392 && yy>=374 && yy<=381)
			 {
				$('#tooltip-span').text("Borehole point(650,380)");    
				$('#tooltip-span').css({
				  position:'absolute',
				  top:event.pageY-10,
				  left:event.pageX-90,
...
function magic()
{
	if(simsubscreennum == 1)
	{
		...
		setTimeout(function()
		{
			animateArrowATPosition(312,230,270);
		...
		setTimeout(function()
		{
			...
			setTimeout(function()
			{
				...
				generateQuestion(q1,"Borehole location is identified at _________ region","","Conglomerate","Shale","Limestone","Banded Shale",4,nextButtonProceed,200,300,300,120);
			},3000);
		},800);
	...
	else if(simsubscreennum == 11)
	{
		document.getElementById('nextButton').style.visibility="hidden";
		document.getElementById('cmap').style.visibility="visible";	
		document.getElementById('cmap').innerHTML = "Mark Borehole Point";
		spanclick=13;	
...
function markBoreholePoint()
{
	...
	ctx.strokeStyle="red";
	ctx.fillStyle="blue";
	ctx.arc(grid_size*(650/100),-grid_size*(380/100),5,0,2*Math.PI,true);
	ctx.fill();
	ctx.stroke();
	ctx.closePath();
	document.getElementById("nextButton").style.visibility = "visible";
...
function markBoreholeLine()
{
	...
	if (borey==185) 
	{
		...
		generateQuestion(q2,"What is the depth of borehole?","","150m","185m","350m","120m",2,nextButtonProceed,100,150,300,120);
		ctx.closePath();
	}
}
...
function boreHoleNaming(ptx,pty)
{
	...
	ctx.moveTo(grid_size*(650/100),-grid_size*(380/100));
	ctx.lineTo(grid_size*(720/100),-grid_size*(380/100));
	ctx.moveTo(grid_size*(ptx/100),-grid_size*(pty/100));
	ctx.lineTo(grid_size*((ptx+70)/100),-grid_size*(pty/100));
	ctx.moveTo(grid_size*(700/100),-grid_size*(380/100));
	ctx.lineTo(grid_size*((ptx+50)/100),-grid_size*(pty/100));
	ctx.fillText("185m",grid_size*(720/100),-grid_size*(250/100),30);
	ctx.stroke();
}
...
function drawppattern()
{
	...
	ctx.fillText("+",grid_size*(15/100),-(grid_size*(95/100)));
	ctx.fillText("+",grid_size*(55/100),-(grid_size*(95/100)));
	ctx.fillText("+",grid_size*(85/100),-(grid_size*(95/100)));
...
function drawqpattern()
{
	...
	// ctx.fill();
	for(i=120;i<190;i+=50)
	{
		ctx.beginPath();
		ctx.arc(grid_size*(i/100),-grid_size*(80/100),3,0,2*Math.PI,false);
		ctx.fill();
	}
	for(i=140;i<210;i+=50)
	{
		ctx.beginPath();
		ctx.arc(grid_size*(i/100),-grid_size*(100/100),3,0,2*Math.PI,false);
		ctx.fill();
	}
 ...
 function drawspattern()
 {
	...
	ctx.drawImage(sp,grid_size*(600/100), -grid_size*(365/100));
	ctx.drawImage(sp,grid_size*(650/100), -grid_size*(365/100));
